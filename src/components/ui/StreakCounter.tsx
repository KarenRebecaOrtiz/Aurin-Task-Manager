...
interface StreakCounterProps {
  className?: string;
  userId?: string; // Para mostrar el streak de un usuario específico
}

const StreakCounter: React.FC<StreakCounterProps> = ({ className, userId }) => {
  const { user: currentUser } = useUser();
  const { streakData, updateStreak, isLoading } = useStreak(userId);
  const [showAnimation, setShowAnimation] = useState(false);
  const [isVisible, setIsVisible] = useState(false);

  // Actualizar streak cuando el componente se monta (solo si es el propio usuario)
  useEffect(() => {
    if (!isLoading && !isVisible) {
      // Solo actualizar streak si es el propio usuario
      if (!userId || userId === currentUser?.id) {
        updateStreak();
      }
      setIsVisible(true);
    }
  }, [isLoading, isVisible, updateStreak, userId, currentUser?.id]);

  // Mostrar animación cuando el streak cambia
  useEffect(() => {
  ...
  if (streakData.currentStreak === 0) {
    return null; // No mostrar si no hay streak (streak = 0 significa sin racha)
  }
        ...
        <div className={styles.tooltip}>
          <div className={styles.tooltipContent}>
            <div className={styles.tooltipRow}>
              ...
              <strong>{streakData.currentStreak} días</strong>
            </div>
            <div className={styles.tooltipRow}>
              <span>Mejor racha:</span>
              <strong>{streakData.longestStreak} días</strong>
            </div>
            <div className={styles.tooltipRow}>
              <span>Total de días:</span>
              <strong>{streakData.totalAccessDays}</strong>
            </div>
          </div>
        </div>
      </motion.div>
