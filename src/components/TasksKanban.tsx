...
const TasksKanban: React.FC<TasksKanbanProps> = memo(
  ...
  useEffect(() => {
    ...
    // Cleanup for disappearance (on unmount)
    return () => {
      gsap.to(container, {
        opacity: 0,
        y: -50,
  ...
  useEffect(() => {
    const fetchAdminStatus = async () => {
      if (!userId) {
        console.warn('[TasksKanban] No userId, skipping admin status fetch');
        setIsAdmin(false);
        return;
      }
      try {
        console.log('[TasksKanban] Fetching admin status for user:', userId);
        const userDoc = await getDoc(doc(db, 'users', userId));
        if (userDoc.exists()) {
          const access = userDoc.data().access;
          setIsAdmin(access === 'admin');
          console.log('[TasksKanban] Admin status fetched:', {
            userId,
            access,
          ...
          setIsAdmin(false);
          console.warn('[TasksKanban] User document not found for ID:', userId);
        }
      } catch (error) {
        console.error('[TasksKanban] Error fetching admin status:', {
