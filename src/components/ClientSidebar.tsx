...
const ChatSidebar: React.FC<ChatSidebarProps> = ({
  ...
  useEffect(() => {
    const fetchAdminStatus = async () => {
      if (!user?.id) {
        setIsAdmin(false);
        console.log('[ChatSidebar:FetchAdminStatus] No user ID, setting isAdmin to false');
        return;
      }
      try {
        console.log('[ChatSidebar:FetchAdminStatus] Fetching admin status for user', user.id);
        const userDoc = await getDoc(doc(db, 'users', user.id));
        if (userDoc.exists()) {
          setIsAdmin(userDoc.data().access === 'admin');
          console.log('[ChatSidebar:FetchAdminStatus] Admin status fetched', { isAdmin: userDoc.data().access === 'admin' });
        } else {
          setIsAdmin(false);
          console.log('[ChatSidebar:FetchAdminStatus] User document does not exist, setting isAdmin to false');
        }
      } catch (error) {
        console.error('[ChatSidebar:FetchAdminStatus] Error fetching admin status', error);
        setIsAdmin(false);
      }
    };
    fetchAdminStatus();
  ...
  useEffect(() => {
    if (!task.id) {
      setIsLoading(false);
      console.log('[ChatSidebar:TaskListener] No task ID, setting isLoading to false');
      return;
    }
    console.log('[ChatSidebar:TaskListener] Setting up task listener for task', task.id);
    const unsubscribe = onSnapshot(
      ...
      (doc) => {
        if (doc.exists()) {
          ...
          console.log('[ChatSidebar:TaskListener] Task updated', { taskId: doc.id });
        }
      },
      (error) => {
    ...
    return () => {
      console.log('[ChatSidebar:TaskListener] Unsubscribing task listener');
      unsubscribe();
    };
  }, [task.id]);
  ...
  useEffect(() => {
    if (!task.id || !user?.id) {
      console.log('[ChatSidebar:TimerListener] No task ID or user ID, skipping timer listener', { taskId: task.id, userId: user?.id });
      return;
    }
    console.log('[ChatSidebar:TimerListener] Setting up timer listener for user', user.id, 'task', task.id);
      ...
      (doc) => {
        if (doc.exists()) {
          ...
          if (timerData.isRunning && timerData.startTime) {
            ...
            console.log('[ChatSidebar:TimerListener] Timer updated', { isRunning: timerData.isRunning, elapsedSeconds });
          } else {
            setTimerSeconds(timerData.accumulatedSeconds);
            console.log('[ChatSidebar:TimerListener] Timer stopped', { accumulatedSeconds: timerData.accumulatedSeconds });
          }
        } else {
          setDoc(timerDocRef, {
            userId: user.id,
            ...
            console.error('[ChatSidebar:TimerListener] Error initializing timer', error);
          });
          console.log('[ChatSidebar:TimerListener] Timer document created');
        }
      },
      (error) => {
        console.error('[ChatSidebar:TimerListener] Error listening to timer', error);
      },
    );
    return () => {
      console.log('[ChatSidebar:TimerListener] Unsubscribing timer listener');
      unsubscribe();
    };
  }, [task.id, user?.id]);
  ...
  useEffect(() => {
    if (!task.id) {
      setIsLoading(false);
      console.log('[ChatSidebar:MessagesListener] No task ID, setting isLoading to false');
      return;
    }
    console.log('[ChatSidebar:MessagesListener] Setting up messages listener for task', task.id);
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setMessages((prev) => {
          const pendingMessages = prev.filter((msg) => msg.isPending || msg.hasError);
          return [...pendingMessages, ...newMessages.filter((msg) => !msg.isPending)];
        });
        setIsLoading(false);
        console.log('[ChatSidebar:MessagesListener] Messages updated', { messageCount: newMessages.length });
      },
      (error) => {
        console.error('[ChatSidebar:MessagesListener] Error fetching messages', error);
        setIsLoading(false);
      },
    );
    return () => {
      console.log('[ChatSidebar:MessagesListener] Unsubscribing messages listener');
      unsubscribe();
    };
  }, [task.id]);
  ...
  useEffect(() => {
    if (isOpen && user?.id) {
      ...
      console.log('[ChatSidebar:MarkRead] Marking unread messages as read', { unreadCount: unreadMessages.length });
      unreadMessages.forEach(async (msg) => {
        try {
          await updateDoc(doc(db, `tasks/${task.id}/messages`, msg.id), {
            read: true,
          });
          console.log('[ChatSidebar:MarkRead] Message marked as read', msg.id);
        } catch (error) {
          console.error('[ChatSidebar:MarkRead] Error marking message as read', error);
        }
      });
    }
    ...
    if (newUnreadMessages.length > 0 && user?.id && hasInteracted) {
      ...
      if (latestMessage.senderId !== user.id) {
        audioRef.current.play().catch((error) => {
          console.error('[ChatSidebar:NotificationSound] Error playing notification sound', error);
        });
        console.log('[ChatSidebar:NotificationSound] Playing notification for new message', latestMessage.id);
      }
    }
  ...
  const handleStatusChange = async (status: string) => {
    if (!isCreator && !isAdmin) {
      console.log('[ChatSidebar:StatusChange] User not authorized to change status', { isCreator, isAdmin });
      return;
    }
    try {
      console.log('[ChatSidebar:StatusChange] Updating task status', { taskId: task.id, status });
      ...
      for (const recipientId of Array.from(recipients)) {
        ...
        console.log('[ChatSidebar:StatusChange] Notification sent to', recipientId);
      }
      console.log('[ChatSidebar:StatusChange] Task status updated successfully');
    } catch (error) {
      console.error('[ChatSidebar:StatusChange] Error updating task status', error);
    }
  };

  const handleEditTask = () => {
    if (!isCreator && !isAdmin) {
      console.log('[ChatSidebar:EditTask] User not authorized to edit task', { isCreator, isAdmin });
      return;
    }
    router.push(`/dashboard/edit-task?taskId=${task.id}`);
    setIsTaskMenuOpen(false);
    console.log('[ChatSidebar:EditTask] Navigating to edit task', task.id);
  };

  const handleDeleteTask = async () => {
    if (!user?.id || deleteConfirm.toLowerCase() !== 'eliminar') {
      console.log('[ChatSidebar:DeleteTask] Invalid user or confirmation', { userId: user?.id, deleteConfirm });
      return;
    }
    setIsDeleting(true);
    try {
      console.log('[ChatSidebar:DeleteTask] Deleting task', task.id);
      await deleteTask(task.id, user.id, isAdmin, task);
      setIsDeletePopupOpen(false);
      setDeleteConfirm('');
      onClose();
      console.log('[ChatSidebar:DeleteTask] Task deleted successfully');
    } catch (error) {
      console.error('[ChatSidebar:DeleteTask] Error deleting task', error);
  ...
  const handleSendMessage = async (
    ...
    if (!user?.id) {
      console.log('[ChatSidebar:HandleSendMessage] No user ID, aborting send');
      return;
    }
    setIsSending(true);
    console.log('[ChatSidebar:HandleSendMessage] Send initiated', { userId: user.id, messageData });
    ...
    setHasInteracted(true);
    console.log('[ChatSidebar:HandleSendMessage] Optimistic message added', { tempId });

    try {
      console.log('[ChatSidebar:HandleSendMessage] Sending message to Firestore');
      ...
      console.log('[ChatSidebar:HandleSendMessage] Message sent to Firestore, updated optimistic message', { docId: docRef.id });
      ...
      for (const recipientId of Array.from(recipients)) {
        ...
        console.log('[ChatSidebar:HandleSendMessage] Notification sent to', recipientId);
      }
    } catch (error) {
      console.error('[ChatSidebar:HandleSendMessage] Send message error', error);
      setMessages((prev) =>
      ...
      setIsSending(false);
      console.log('[ChatSidebar:HandleSendMessage] Send completed', { isSending: false });
    }
  };

  const handleEditMessage = async (messageId: string) => {
    if (!user?.id || !editingText.trim()) {
      console.log('[ChatSidebar:EditMessage] Invalid user or empty text', { userId: user?.id, hasText: !!editingText.trim() });
      return;
    }
    try {
      console.log('[ChatSidebar:EditMessage] Updating message', messageId);
      ...
      setEditingText('');
      console.log('[ChatSidebar:EditMessage] Message updated successfully');
    } catch (error) {
      console.error('[ChatSidebar:EditMessage] Error editing message', error);
    }
  };
  ...
  const toggleTimer = async (e: React.MouseEvent) => {
    if (!user?.id || !task.id) {
      console.log('[ChatSidebar:ToggleTimer] Invalid user or task ID', { userId: user?.id, taskId: task.id });
      return;
    }
    ...
    setHasInteracted(true);
    console.log('[ChatSidebar:ToggleTimer] Timer toggled', { wasRunning, isRunning: !wasRunning });
    ...
    if (wasRunning && timerSeconds > 0) {
      ...
      try {
        console.log('[ChatSidebar:ToggleTimer] Adding time entry', { timeEntry, hours });
        ...
        setTimerSeconds(0);
        console.log('[ChatSidebar:ToggleTimer] Time entry added and timer reset');
      } catch (error) {
        console.error('[ChatSidebar:ToggleTimer] Error adding time entry', error);
      }
    } else if (!wasRunning) {
      try {
        ...
        console.log('[ChatSidebar:ToggleTimer] Timer started');
      } catch (error) {
        console.error('[ChatSidebar:ToggleTimer] Error starting timer', error);
      }
    }
  };
  ...
  const toggleTimerPanel = (e: React.MouseEvent) => {
    ...
    setHasInteracted(true);
    console.log('[ChatSidebar:ToggleTimerPanel] Timer panel toggled', { isOpen: !isTimerPanelOpen });
  };

  const handleAddTimeEntry = async () => {
    if (!user?.id) {
      console.log('[ChatSidebar:AddTimeEntry] No user ID, aborting');
      alert('No se puede añadir la entrada de tiempo: usuario no autenticado.');
      return;
    }
    const [hours, minutes] = timerInput.split(':').map(Number);
    if (isNaN(hours) || isNaN(minutes)) {
      console.log('[ChatSidebar:AddTimeEntry] Invalid time format', timerInput);
    ...
    console.log('[ChatSidebar:AddTimeEntry] Adding time entry', { timeEntry, date, totalHours });
    ...
    try {
      ...
      if (commentInput.trim()) {
        ...
        console.log('[ChatSidebar:AddTimeEntry] Comment added', commentInput.trim());
      }
      setTimerInput('00:00');
      ...
      setHasInteracted(true);
      console.log('[ChatSidebar:AddTimeEntry] Time entry added successfully');
    } catch (error) {
      console.error('[ChatSidebar:AddTimeEntry] Error adding time entry', error);
      ...
      return (
        <div className={styles.imageWrapper}>
          <Image
            ...
            onError={(e) => console.warn('[ChatSidebar:RenderMessage] Image load failed', message.imageUrl)}
          />
        </div>
      );
    }
        ...
        {messages.map((message) => (
          <div
            ...
            className={`${styles.message} ${message.isPending ? styles.pending : ''} ${
              message.hasError && message.senderId === user?.id ? styles.error : ''
            }`}
          >
            <Image
              ...
              onError={(e) => {
                ...
                console.warn('[ChatSidebar:RenderMessage] Avatar load failed', message.senderId);
              }}
            />
