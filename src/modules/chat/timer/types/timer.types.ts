...
// ============================================================================
// TIME TRACKING INTERFACES (Task-level)
// ============================================================================

/**
 * Time tracking summary stored at task level
 * Provides quick access to totals without querying time_logs subcollection
 *
 * @interface TimeTracking
 */
export interface TimeTracking {
  /** Total hours logged (calculated sum) */
  totalHours: number;
  /** Total minutes for precision (remainder after hours) */
  totalMinutes: number;
  /** Timestamp of the last time log entry */
  lastLogDate: Timestamp | null;
  /** 
   * Hours by member for quick access
   * @deprecated Use time_logs subcollection for detailed per-user data
   * Kept for backward compatibility with existing memberHours field
   */
  memberHours?: { [userId: string]: number };
}

/**
 * Individual time log entry stored in tasks/{taskId}/time_logs subcollection
 * Each entry represents a single time registration by a user
 *
 * @interface TimeLog
 */
export interface TimeLog {
  /** Unique log ID (auto-generated by Firestore) */
  id: string;
  /** User ID who registered the time */
  userId: string;
  /** User display name (denormalized for quick display) */
  userName: string;
  /** Start time of the work session (optional for manual entries) */
  startTime: Timestamp | null;
  /** End time of the work session (optional for manual entries) */
  endTime: Timestamp | null;
  /** Duration in minutes (primary unit for calculations) */
  durationMinutes: number;
  /** Description of work performed */
  description: string;
  /** Date string for the log (YYYY-MM-DD format for grouping) */
  dateString: string;
  /** When this log was created */
  createdAt: Timestamp;
  /** Source of the time log */
  source: TimeLogSource;
}

/**
 * Source type for time logs
 * Helps distinguish between automatic timer logs and manual entries
 */
export type TimeLogSource = 'timer' | 'manual' | 'legacy';

/**
 * Input data for creating a new time log
 * Used by createTimeLog function
 */
...
export interface TimerCounterProps {
  ...
  onClick?: () => void;
  disabled?: boolean;
  /** Pill mode - compact horizontal layout matching button heights (32px) */
  pill?: boolean;
}
